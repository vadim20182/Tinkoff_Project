...
class ChannelMessagesAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MessageBaseViewHolder {
        return when (viewType) {
            ...
            TYPE_SEPARATOR -> {
                val binding = ChannelChatSeparatorBinding.inflate(
                    LayoutInflater.from(parent.context), parent, false
                )
                SeparatorViewHolder(binding, onItemClickedListener)
            }
            else -> {
    ...
    override fun onBindViewHolder(holder: MessageBaseViewHolder, position: Int) {
        ...
        uiModel.let {
            when (uiModel) {
                ...
                is UiModel.SeparatorMessageItem -> (holder as SeparatorViewHolder).bind(uiModel.topic)
                else -> {}
            }
        }
    }
    ...
    interface OnItemClickedListener {
        ...
        fun onSeparatorClicked(topic: String)
...
private class MessageViewHolder(
    ...
    private val onItemClickedListener: ChannelMessagesAdapter.OnItemClickedListener
) :
...
private class SeparatorViewHolder(
    private val binding: ChannelChatSeparatorBinding,
    private val onItemClickedListener: ChannelMessagesAdapter.OnItemClickedListener
) : MessageBaseViewHolder(binding.root) {
    private val topicName: TextView = binding.channelChatSeparatorTopic

    fun bind(topic: String) {
        topicName.text = topic
        binding.channelChatSeparatorLayout.setOnClickListener {
            onItemClickedListener.onSeparatorClicked(topic)
        }
    }
